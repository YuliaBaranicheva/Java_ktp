...
public class AStarApp {
    ...
    /**
     * Это 2D-массив компонентов пользовательского интерфейса, которые обеспечивают отображение
     ...
     * изменяя ячейки на основе состояния кнопки мыши и
     * выполненного первоначального редактирования.
     **/
    private class MapCellHandler implements MouseListener
    {
        /**
         * Это значение будет равно true, если была нажата кнопка мыши, и
         ...
         * непроходимыми.  Что это такое, зависит от исходного состояния ячейки,
         * в которой была запущена операция.
         **/
        ...
        public void mousePressed(MouseEvent e)
        {
            ...
            // Если текущая ячейка проходима, то мы создаем их
            // непроходимые; если они непроходимы, то мы делаем их проходимыми.
        ...
        /**
         * Если мышь была нажата, это продолжает
         * операцию изменения в новую ячейку.
         **/
    ...
    /**
     * Создает новый экземпляр приложения Star с заданными шириной и высотой карты.
     **/
    public AStarApp(int w, int h) {
        if (w <= 0)
            throw new IllegalArgumentException("ширина должна быть > 0; получено " + w);

        if (h <= 0)
            throw new IllegalArgumentException("высота должна быть > 0; получено " + h);
     ...
     * состояние карты. Реализация происходит довольно медленно; создается новый объект {@link Map2D},
     * который инициализируется из текущего состояния приложения. Затем вызывается навигатор A*,
     * и если путь найден, дисплей обновляется, чтобы показать найденный путь.
     * (Лучшим решением было бы использовать Model View Controller.)
     **/
    private void findAndShowPath()
    {
        ...
        // Попробуйте вычислить путь. Если один из них может быть вычислен, отметьте все ячейки в
        // пути
